...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune a lightweight LLM to generate PDG graphs only."
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only load and format PDG samples; do not load a model or start training.",
    )
    parser.add_argument("--preview-samples", type=int, default=2)
    args = parser.parse_args()
    model_suffix = model_suffix_from_name(args.model_name)
    args.output_dir = args.output_dir or str(ROOT_DIR / "outputs" / f"single_pdg_{model_suffix}")
    args.save_dir = args.save_dir or str(ROOT_DIR / f"lora_model_single_pdg_{model_suffix}")
    return args


def pick_pdg_column(fieldnames: list[str] | None) -> str:
    if not fieldnames:
        raise ValueError("The training CSV has no header row.")
    if "PDG" in fieldnames:
        return "PDG"
    if "pdg" in fieldnames:
        return "pdg"
    raise ValueError("The training CSV must contain a PDG or pdg column.")


def load_pdg_examples(
    csv_file: str,
    ...
    with open(csv_file, mode="r", encoding="utf-8", newline="") as f:
        ...
        pdg_column = pick_pdg_column(reader.fieldnames)
        ...
        for row in reader:
            ...
            if not code or not pdg:
                continue

            examples.append(
                {
                    "instruction": PDG_INSTRUCTION,
                    "input": code,
                    "output": pdg,
                }
            )
    ...
    if not examples:
        raise ValueError(f"No valid PDG examples were loaded from {csv_file}.")
    return examples
