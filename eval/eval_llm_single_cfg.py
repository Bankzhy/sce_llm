...
def default_output_file(model_dir: str, languages: set[str] | None) -> str:
    ...
    return str(ROOT_DIR / "eval" / f"predict_single_cfg_{suffix}_{language_suffix(languages)}.csv")
...
def load_examples(
    ...
    with open(csv_file, mode="r", encoding="utf-8", newline="") as f:
        ...
        required_columns = {"code", "CFG", "language", "is_error"}
...
def generate_cfg(model, tokenizer, code: str, max_new_tokens: int) -> str:
